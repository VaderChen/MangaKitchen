...
- 受理された領域は対話 BBOX を検索範囲として原画像の連結成分からピクセル文字マスクへ精修し、文字領域を膨張前の実際のglyph外接矩形へ縮小します。自動組版の方向は実際の字形排列を優先します。分類または転記が欠けた不完全な結果は保存しません。
...
- `imageToText`モデルはGUIのローカルステップ3に必須です。ステップ2はVLM未読込でもCore MLのBBOX／形状検出とピクセルマスクを完了でき、ステップ3で候補を分類・転記してからページ文脈翻訳を行います。モデル未読込時にシステムOCRへfallbackせず、モデルを読み込むかMCP Agentから領域と原文を提供します。分類、転記、翻訳はbatch化して全領域の結果を検証し、効果音は主処理に含めません。

...
- 同梱の manga109 吹き出し分割 Core ML モデルが、Apple Neural Engine を優先して白黒漫画の対話 BBOX と吹き出し形状を生成します。形状でマスク検索範囲を切り、HTML組版用の内接矩形も計算します。その後、画像テキスト化モデルが各候補を分類・転記します。文字認識や位置決めにシステムOCRは使用しません。効果音、ページ番号、フッター情報、人物、空白領域は主処理から除外します。
- PP-OCRv6 Small のネイティブ Swift／Core ML OCR recognizer も同梱します。VLM が既存領域を決め、OCR は候補文字、信頼度、行枠、読み方向を `ocrResults` に保存するだけで、原文・座標・マスクを上書きしません。
...
3. **翻訳と組版**：GUIはVLMを使用し、MCPはAppが作成した単一ページの作業パッケージをAgentへ渡して原文抽出・翻訳・組版を行い、結果をAppのプロジェクト状態へ戻します。
...
- `imageToText`モデルはGUIのローカルステップ3に必須です。ステップ2はVLM未読込でもCore MLのBBOX／形状検出とピクセルマスクを完了でき、ステップ3で候補を分類・転記してからページ文脈翻訳を行います。モデル未読込時にシステムOCRへfallbackせず、モデルを読み込むかMCP Agentから領域と原文を提供します。分類、転記、翻訳は領域ごとに例外を分離するため、1領域の失敗でページ全体が失敗せず、効果音は主処理に含めません。
...
- 同梱の `manga109-segmentation-bubble` と PP-OCRv6 Small Core ML モデルは Apache-2.0 の元モデルから生成しています。元の画像テキスト化重み、超解像、画像変換モデルの重みは同梱しておらず、容量、ライセンス、配布方法は別途扱います。OCR の変換済み `.mlpackage` とライセンス声明は repository に含まれます。

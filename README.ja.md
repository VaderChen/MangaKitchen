...
- 同梱の manga109 吹き出し分割 Core ML モデルが、Apple Neural Engine を優先して白黒漫画の対話 BBOX 候補を生成し、画像テキスト化モデルが各候補を分類・転記します。文字認識や位置決めにシステムOCRは使用しません。効果音、ページ番号、フッター情報、人物、空白領域は主処理から除外します。
- VLMが受理した領域は対話 BBOX を検索範囲として原画像の連結成分から文字形状polygonへ精修し、文字領域も実際のglyph外接矩形へ縮小します。分類または転記が欠けた不完全な結果は保存しません。
...
2. **文字とマスク**：閉領域候補を検出し、ローカルVLMで分類・転記した後、文字形状maskへ精修します。MCP Agentは領域と原文を直接提供でき、利用者はmaskを追加・消去・修正できます。
3. **翻訳と組版**：各領域の原文、訳文、位置、フォント、サイズを画像に対応する`.str`へ保存します。
4. **修復と合成**：元の文字を除去し、背景を修復して訳文を組版し、プロジェクトの出力先へ保存します。

4段階は再開可能な状態、生成物、依存関係を定義するもので、毎回ステップ1からやり直す固定チェックリストではありません。GUIとMCPはページ状態と`.str`を確認し、前提データがある任意の段階から開始できます。マスクがあれば翻訳へ、訳文があれば組版や合成へ直接進めます。明示的に再実行を求めない限り、完了済みの領域認識、マスク、訳文、手動編集を上書きしません。
...
- ステップ3の前に、原稿ページ、文字領域、VLM／Agent／利用者が確認した原文、マスクを確認します。不完全ならステップ2へ戻ります。
...
- `imageToText`モデルはGUIのローカルステップ2に必須です。閉領域のタイトル／台詞候補を分類して原文を転記し、ステップ3ではページ文脈翻訳を担当します。モデル未読込時にシステムOCRへfallbackせず、モデルを読み込むかMCP Agentから領域と原文を提供します。分類、転記、翻訳はbatch化して全領域の結果を検証し、効果音は主処理に含めません。
...
ローカル`imageToText`モデルが読み込まれている場合、`region_source`を`local`に設定するとステップ2で閉領域検出とVLM転記を利用できます。MCPの訳文は引き続きAgentが`region.update`で提供します。純粋なAgentモードは`detect_masks → supplement_regions → region.update → compose`を使用します。
...
- 同梱の `manga109-segmentation-bubble` Core ML モデルは Apache-2.0 の元モデルから生成しています。画像テキスト化と画像変換のモデルweightは同梱しておらず、容量、ライセンス、配布方法は別途扱います。
- 対話 BBOX と Agent の粗枠は原画像の明度・連結成分で精修します。暗色／カラーの作品や対話以外の文字は、`DialogueRegion` の形式を変えずに精密な Agent polygon で補えます。効果音は意図的に翻訳主処理から外しています。

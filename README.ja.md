...
Appは`mlx-swift-lm`対応のローカルマルチモーダル翻訳モデルを`MLXVLMRuntime`で公開します。`MLXTextRuntime`は旧契約互換のためだけに残り、UIでは選択できません。推奨モデルは約3GBの`lmstudio-community/Qwen3.5-4B-MLX-4bit`です。

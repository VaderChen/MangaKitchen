...
enum MLXGGUFModelSource {
    ...
    static func weightURL(
        in directoryURL: URL,
        manifest: ModelManifest
    ) throws -> URL? {
        let ggufFiles = files(in: directoryURL, withExtension: "gguf")
        let safetensorFiles = files(in: directoryURL, withExtension: "safetensors")
        let requestedFile: String?
        if let weightsFile = manifest.weightsFile {
            if weightsFile.lowercased().hasSuffix(".gguf") {
                requestedFile = weightsFile
            } else if manifest.weightsFormat == .gguf {
                requestedFile = weightsFile
            } else {
                requestedFile = nil
            }
        } else if let modelFile = manifest.modelFile,
                  modelFile.lowercased().hasSuffix(".gguf") {
            requestedFile = modelFile
        } else {
            requestedFile = nil
        }

        if let requestedFile {
            let candidate = try resolvedFile(
                requestedFile,
            ...
            guard candidate.pathExtension.lowercased() == "gguf",
                ...
                throw MLXGGUFLoaderError.ambiguousWeights(candidate)
            }
            return try validatedWeightURL(candidate)
        }

        switch manifest.weightsFormat {
        case .safetensors:
            return nil
        case .gguf:
            guard ggufFiles.count == 1 else {
                throw MLXGGUFLoaderError.ambiguousWeights(directoryURL)
            }
            return try validatedWeightURL(ggufFiles[0])
        case nil:
            guard safetensorFiles.isEmpty else { return nil }
            guard ggufFiles.count == 1 else {
                if ggufFiles.isEmpty { return nil }
                throw MLXGGUFLoaderError.ambiguousWeights(directoryURL)
            }
            return try validatedWeightURL(ggufFiles[0])
        }
    ...
    private static func validatedWeightURL(_ candidate: URL) throws -> URL {
        let inspection = try MLXNativeGGUFBackend().inspect(fileURL: candidate)
    ...
    static func mmprojURL(
        in directoryURL: URL,
        manifest: ModelManifest
    ) throws -> URL? {
        guard let mmprojFile = manifest.mmprojFile else { return nil }
        let candidate = try resolvedFile(mmprojFile, relativeTo: directoryURL)
        guard candidate.pathExtension.lowercased() == "gguf",
              FileManager.default.fileExists(atPath: candidate.path) else {
            throw MLXGGUFLoaderError.missingMultimodalProjector(candidate)
        }
        return candidate
    }
    ...
    private static func resolvedFile(_ path: String, relativeTo directoryURL: URL) throws -> URL {
        let candidate = directoryURL.appendingPathComponent(path).standardizedFileURL

...
- 정규화 벡터 브러시로 마스크 추가, 지우기, 영역별 실행 취소 후 이진 PNG를 생성합니다. 2단계 완료 후 이미지→이미지 모델을 시작하지 않고 원문을 제거한 CPU／GPU 마스크 확인 미리보기를 즉시 표시합니다.
- 흑백 만화에서는 시스템 Vision OCR과 닫힌 밝은 영역 감지를 서로 독립적으로 실행합니다. 각 OCR 거친 영역은 가장 적합한 닫힌 후보에 할당하고, 할당되지 않은 영역은 테두리 없는 텍스트로 유지합니다. OCR이 아무것도 찾지 못한 닫힌 후보도 모두 이미지→텍스트 모델에 전달한 뒤 OCR, VLM 출력 및 닫힌 영역 형상을 병합하고 중복을 제거합니다. 현재 주 처리 흐름에서는 효과음, 페이지 번호, 하단 정보, 인물 및 빈 영역을 의도적으로 제외합니다.
- 후보 텍스트는 연결 요소로 글자 다각형까지 정밀화한 뒤 전체 페이지 문맥을 유지한 배치로 OCR을 교정합니다. 원문과 교정값을 모두 저장하며 모든 영역 결과가 있을 때만 완료 상태로 기록합니다.
...
- 대화 텍스트 마스크는 하나 이상의 다각형으로 원문을 덮고 말풍선 경계에서 잘린 뒤 브러시 추가·지우기를 겹칠 수 있습니다. 이미지 모델이 없으면 CPU 또는 Metal GPU 복원을 선택할 수 있습니다.
- Core Text는 원문 glyph의 위치와 크기를 기준점으로 사용하며 가로／세로 조판, overflow 우선 축소, 같은 말풍선의 여러 영역을 겹치지 않는 레인으로 분할, 말풍선 내부 hard clip 및 수동 수정 후 재배치를 지원합니다.
...
2. **텍스트와 마스크**: 대화 영역과 원문을 감지하고 거친 상자를 글자 마스크로 정밀화한 뒤 로컬 LLM, Agent 또는 사람이 OCR을 한 번 교정하고 마스크를 편집합니다.
...
- 3단계 전에 원본 페이지, 텍스트 영역, 한 번 교정된 OCR 원문 및 마스크를 확인합니다. 불완전하면 2단계로 돌아갑니다.
...
- `imageToText` 모델은 2단계에서 제목／대화 후보를 분류하고 OCR이 놓친 닫힌 영역을 보완하여 OCR을 완전히 교정하며, 3단계에서 페이지 문맥 번역을 담당합니다. 먼저 시스템 OCR, VLM 출력 및 닫힌 영역 형상을 병합하고 중복을 제거합니다. 모델을 불러오지 않아도 시스템 OCR만으로 마스크를 만들 수 있습니다. 효과음은 현재 번역 주 처리 흐름에 포함하지 않고, 교정과 번역은 배치 처리하여 모든 영역의 결과를 검증합니다.
...
4. 기존 영역의 `ocrTextRefined`가 false이면 Agent가 `rawSourceText`를 먼저 교정한 뒤 번역하고 `mangakitchen.region.update`로 `source_text`, 번역문 및 조판 설정을 기록합니다.
5. `mangakitchen.page.compose`로 배경 복원과 출력을 수행합니다.

로컬 `imageToText` 모델도 로드되어 있으면 Agent가 `mangakitchen.page.translate` 또는 `mangakitchen.page.run_full`을 사용할 수 있습니다. `page.run_full`에는 로컬 이미지→텍스트 모델이 필요합니다. 순수 Agent 번역은 `detect_masks → supplement_regions → region.update → compose`를 사용하며 누락 영역이 없으면 `supplement_regions`를 생략할 수 있습니다.
...
- 현재 자동 후보 감지는 흑백 만화의 닫힌 밝은 영역을 중심으로 합니다. 어둡거나 컬러인 말풍선과 닫히지 않은 내레이션 상자는 향후 segmentation 모델로 보강할 수 있습니다. 효과음은 의도적으로 번역 주 처리 흐름에서 제외하며, 향후 지원 시 별도 감지·조판 방식을 사용합니다.

...
- 승인된 영역은 대화 BBOX를 검색 범위로 사용하여 원본 이미지 픽셀에서 글자 픽셀 마스크로 정밀화하고, 텍스트 영역도 팽창 전 실제 glyph 경계로 축소합니다. 자동 조판 방향은 실제 글자 배열 감지 결과를 우선합니다. 분류 또는 전사가 누락된 불완전한 결과는 저장하지 않습니다.
...
- `imageToText` 모델은 GUI 로컬 3단계에 필수입니다. 2단계는 VLM이 없어도 Core ML BBOX／형상 감지와 픽셀 마스크를 완료하며, 3단계에서 후보를 분류·전사한 뒤 페이지 문맥 번역을 수행합니다. 모델이 없을 때 시스템 OCR로 fallback하지 않으므로 모델을 불러오거나 MCP Agent가 영역과 원문을 제공해야 합니다. 분류, 전사 및 번역은 배치 처리하여 모든 영역의 결과를 검증하고 효과음은 주 처리 흐름에서 제외합니다.

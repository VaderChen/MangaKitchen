...
- 일반, 고급, 모델, MCP, 정보 탭으로 구성된 전역 설정 DLG. 언어, 색상 모드, CPU/GPU 이미지 합성, 데이터 위치, 멀티모달／텍스트 위치／OCR／텍스트 생성／초해상도 모델, MCP 포트 및 IP/CIDR 허용 목록을 설정할 수 있습니다.
...
- 번역 방식은 다운로드한 텍스트 생성 모델 또는 멀티모달 모델 중에서 선택합니다. 기본 텍스트 생성 방식은 OCR／VLM이 추출한 원문만 사용하고 페이지 이미지를 읽지 않습니다. 멀티모달 방식은 이미지 문맥, 텍스트 위치 및 원문 추출에도 사용할 수 있으므로 VLM은 전체 워크플로의 필수 조건이 아닙니다.
...
- 텍스트／멀티모달 모델은 실제 사용할 때 지연 로드되고 같은 model container를 재사용합니다. 메모리 사용량이 높으면 큰 모델을 로드하기 전에 다른 runtime을 해제합니다.
...
3. **번역과 조판**: GUI는 내장 OCR 또는 선택한 VLM 경로로 원문을 추출하고, 선택한 텍스트 생성／멀티모달 모델로 번역과 선택적 2차 교정 및 의미 QA를 수행합니다. MCP는 App이 만든 단일 페이지 작업 패키지를 Agent에 전달하여 원문 추출·번역·조판을 수행하며, 결과를 App 프로젝트 상태에 반영합니다.
...
- `imageToText` VLM은 프로젝트가 VLM 원문 추출 또는 멀티모달 번역을 명시적으로 선택할 때만 필요합니다. 2단계 마스크, PP-OCR ‘원문 다시 추출’, 텍스트 생성 모델 번역, 재조판 및 출력은 VLM 없이 동작합니다.
...
`MLXTextRuntime`과 `MLXVLMRuntime`은 `mlx-swift-lm`이 지원하는 로컬 텍스트 생성／멀티모달 모델을 불러옵니다. 텍스트 번역에는 `mlx-community/Qwen3-4B-4bit`, 멀티모달 작업에는 약 3GB인 `lmstudio-community/Qwen3.5-4B-MLX-4bit`를 권장합니다.
...
- 내장 `manga109-segmentation-bubble`, PP-OCRv6 Medium 기본 모델 및 Small fallback Core ML 모델은 Apache-2.0 원본 모델에서 생성했습니다. 원본 이미지→텍스트, 초해상도 및 이미지→이미지 모델 weight는 포함하지 않으며, 크기·라이선스·배포 정책은 별도로 적용합니다. 변환된 OCR `.mlpackage`와 라이선스 고지는 repository에 포함됩니다.

...
- PP-OCRv6 Small 네이티브 Swift／Core ML OCR recognizer도 포함됩니다. VLM이 기존 영역을 결정하고 OCR은 후보 텍스트, 신뢰도, 줄 상자와 읽기 방향을 `ocrResults`에 저장할 뿐 원문·좌표·마스크를 덮어쓰지 않습니다.
...
- 내장 `manga109-segmentation-bubble` 및 PP-OCRv6 Small Core ML 모델은 Apache-2.0 원본 모델에서 생성했습니다. 원본 이미지→텍스트, 초해상도 및 이미지→이미지 모델 weight는 포함하지 않으며, 크기·라이선스·배포 정책은 별도로 적용합니다. 변환된 OCR `.mlpackage`와 라이선스 고지는 repository에 포함됩니다.

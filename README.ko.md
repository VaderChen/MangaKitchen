...
- 내장된 manga109 말풍선 분할 Core ML 모델이 Apple Neural Engine을 우선 사용하여 흑백 만화의 대화 BBOX 후보를 만들고, 이미지→텍스트 모델이 각 후보를 분류하고 전사합니다. 텍스트 인식이나 위치 지정에 시스템 OCR을 사용하지 않습니다. 효과음, 페이지 번호, 하단 정보, 인물 및 빈 영역은 주 처리 흐름에서 제외합니다.
- VLM이 승인한 영역은 대화 BBOX를 검색 범위로 사용하여 원본 이미지의 연결 요소에서 글자 다각형까지 정밀화하고 텍스트 영역도 실제 glyph 경계로 축소합니다. 분류 또는 전사가 누락된 불완전한 결과는 저장하지 않습니다.
...
2. **텍스트와 마스크**: 내장 Core ML 분할 모델로 대화 BBOX 후보를 감지하고 로컬 VLM으로 분류·전사한 뒤 글자 마스크로 정밀화합니다. MCP Agent는 영역과 원문을 직접 제공할 수 있으며 사용자는 마스크를 추가·삭제·수정할 수 있습니다.
3. **번역과 조판**: 각 영역의 원문, 번역문, 위치, 글꼴 및 크기를 이미지의 `.str` 파일에 기록합니다.
4. **복원과 합성**: 원래 글자를 제거하고 배경을 복원한 뒤 번역문을 조판하여 프로젝트 출력 폴더에 저장합니다.

4단계는 재개 가능한 상태, 산출물 및 의존 관계를 정의하며 매번 1단계부터 다시 실행하는 고정 체크리스트가 아닙니다. GUI와 MCP는 페이지 상태와 `.str`을 먼저 확인하고 선행 자료가 있는 임의 단계부터 시작할 수 있습니다. 마스크가 있으면 바로 번역하고, 번역문이 있으면 바로 조판 또는 합성하며, 한 영역만 수정할 수도 있습니다. 사용자나 Agent가 명시적으로 재실행하지 않는 한 완료된 영역 인식, 마스크, 번역문 및 수동 편집을 덮어쓰지 않습니다.
...
- `imageToText` 모델은 GUI 로컬 2단계에 필수입니다. 내장 Core ML 분할 모델의 제목／대화 BBOX 후보를 분류하고 원문을 전사하며, 3단계에서는 페이지 문맥 번역을 담당합니다. 모델이 없을 때 시스템 OCR로 fallback하지 않으므로 모델을 불러오거나 MCP Agent가 영역과 원문을 제공해야 합니다. 분류, 전사 및 번역은 배치 처리하여 모든 영역의 결과를 검증하고 효과음은 주 처리 흐름에서 제외합니다.
...
로컬 `imageToText` 모델이 로드되어 있으면 `region_source`를 `local`로 설정하여 2단계에서 대화 BBOX 감지와 VLM 전사를 사용할 수 있습니다. MCP 번역문은 여전히 Agent가 `region.update`로 제공합니다. 순수 Agent 모드는 `detect_masks → supplement_regions → region.update → compose`를 사용합니다.
...
- 대화 BBOX와 Agent의 대략적인 상자는 원본 이미지의 밝기·연결 요소로 정밀화합니다. 어둡거나 컬러인 작품과 대화 이외의 텍스트는 `DialogueRegion` 형식을 바꾸지 않고 정밀한 Agent polygon으로 보완할 수 있습니다. 효과음은 의도적으로 번역 주 처리 흐름에서 제외합니다.

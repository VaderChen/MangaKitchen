...
enum Qwen3VLLanguage {
    ...
    final class Model: Module {
        ...
        func dflashForward(
            ...
            let mask: MLXArray? = hidden.dim(1) > 1
                ? createCausalMask(n: hidden.dim(1), offset: cache?.first?.offset ?? 0)
                : nil
            ...
            return (norm(hidden), concatenated(captured, axis: -1))
        }
    ...
    final class LanguageModel: Module, KVCacheDimensionProvider {
        ...
        func dflashForward(
            ...
            if (cache?.first?.offset ?? 0) == 0 || state[ropeDeltasKey] == nil || cache == nil {
                ...
                base = broadcast(base[.newAxis, 0...], to: [batch, sequenceLength])
                base = base + delta[0..., .newAxis]
                positionIds = broadcast(
                    base[.newAxis, 0..., 0...],
                    to: [3, batch, sequenceLength])
            }
...
public final class Qwen3VL: Module, VLMModel, KVCacheDimensionProvider,
    ...
    public var dflashLayerCount: Int { config.textConfiguration.numHiddenLayers }
    ...
    public func dflashMultimodalPrefill(
        ...
        let textEmbeds = languageModel.model.embedTokens(inputIds)
        let (visionHidden, deepstackOutputs) = visionModel(pixelValues, gridTHW: frames)
        let mergeSize = config.visionConfiguration.spatialMergeSize
        let splits = frames.map { $0.product / (mergeSize * mergeSize) }
        let splitIndices = cumulativeSplitIndices(from: splits)
        let featureSlices = visionHidden.split(indices: splitIndices)
        let flattenedFeatures = concatenated(featureSlices).asType(textEmbeds.dtype)

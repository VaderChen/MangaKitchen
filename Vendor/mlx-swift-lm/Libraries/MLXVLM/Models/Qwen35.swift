...
enum Qwen35Language {
    ...
    final class GatedDeltaNet: Module {
        ...
        func dflashForward(
        ...
        ) -> (output: MLXArray, rollback: Qwen35VLDFlashLinearCapture?) {
            let B = inputs.dim(0)
            let S = inputs.dim(1)
            ...
            let convolutionInput = concatenated([convState, mixedQKV], axis: 1)
            if let cache, convKernelSize > 1 {
                cache[0] = contiguous(
                    convolutionInput[0..., (-(convKernelSize - 1))..., 0...])
            }

            let convolutionOutput = silu(conv1d(convolutionInput))
            let split = MLX.split(
                convolutionOutput,
                indices: [keyDim, 2 * keyDim],
                axis: -1)
            let q = split[0].reshaped(B, S, numKHeads, headKDim)
            let k = split[1].reshaped(B, S, numKHeads, headKDim)
            let v = split[2].reshaped(B, S, numVHeads, headVDim)
            ...
            let rollback = captureRollback && cache != nil
                ? Qwen35VLDFlashLinearCapture(
                    ...
                    mask: mask)
                : nil
            output = norm(output, gate: z)
            return (outProj(output.reshaped(B, S, -1)), rollback)
        }
    }

    final class Qwen35VLDFlashLinearCapture {
        let attention: GatedDeltaNet
    ...
    final class Qwen35VLDFlashRollback: DFlashTargetRollback {
        let captures: [Qwen35VLDFlashLinearCapture]
        let targetCache: [KVCache]

        init(captures: [Qwen35VLDFlashLinearCapture], targetCache: [KVCache]) {
        ...
        func commit(acceptedInputCount: Int, rejectedInputCount: Int) throws {
            ...
            guard captures.allSatisfy({
                ...
                throw DFlashError.unsupportedGeneration(
                    "Qwen3.5 VLM Gated Delta Net rollback snapshot 不完整。")
            }
            ...
            for cache in targetCache where !(cache is MambaCache) {
                ...
                guard trimmed == rejectedInputCount else {
                    throw DFlashError.unsupportedGeneration(
                        "Qwen3.5 VLM Attention KV Cache 回滾失敗。")
                }
            }
            ...
            for capture in captures {
                ...
                capture.cache[0] = contiguous(
                    capture.convolutionInput[0..., acceptedInputCount ..< convolutionEnd, 0...])
    ...
    final class DecoderLayer: Module {
        ...
        func dflashForward(
            ...
            let r: MLXArray
            var rollback: Qwen35VLDFlashLinearCapture?
            if isLinear {
    ...
    final class Model: Module {
        ...
        func dflashForward(
            ...
            hidden: MLXArray,
            captured: MLXArray,
            rollback: Qwen35VLDFlashRollback?
        ) {
            ...
            var rollbackCaptures = [Qwen35VLDFlashLinearCapture]()

            for (index, layer) in layers.enumerated() {
                let layerSSMMask = layer.isLinear ? ssmMask : nil
                let attentionMask = layer.isLinear ? nil : faMask
            ...
            let rollback = captureRollback && cache != nil
                ? Qwen35VLDFlashRollback(captures: rollbackCaptures, targetCache: cache!)
                : nil
    ...
    final class LanguageModel: Module {
        ...
        func dflashForward(
            ...
            var state = state ?? .init()
            let inputs = inputIds.ndim == 1
                ? inputIds.expandedDimensions(axis: 0)
                : inputIds
                ...
                || cache == nil
            {
                if let precomputedPositionIds {
                    ...
                    positionIds = precomputedPositionIds[
                        0..., 0..., cacheOffset ..< (cacheOffset + sequenceLength)]
                } else {
                    let (computed, deltas) = Qwen3VLLanguage.getRopeIndex(
                        inputIds: inputs,
                        imageGridTHW: imageGridTHW,
                ...
                var base = MLXArray(0 ..< sequenceLength).asType(.int32)
                base = broadcast(base[.newAxis, 0...], to: [batchSize, sequenceLength])
            ...
            let output = model.dflashForward(
                ...
                captureRollback: captureRollback)
            let logits = if let lmHead {
                lmHead(output.hidden)
            } else {
                model.embedTokens.asLinear(output.hidden)
            }
...
public class Qwen35: Module, VLMModel, KVCacheDimensionProvider,
    ...
    public var dflashHiddenSize: Int { config.textConfiguration.hiddenSize }
    public var dflashLayerCount: Int { config.textConfiguration.hiddenLayers }
    ...
    public func dflashComputeLogits(_ hiddenStates: MLXArray) -> MLXArray {
        ...
        return languageModel.model.embedTokens.asLinear(hiddenStates)
    }
    ...
    public func dflashMultimodalPrefill(
        input: LMInput,
        cache: [KVCache]?,
        captureLayerIDs: [Int],
        captureRollback: Bool
    ) throws -> DFlashTargetOutput {
        ...
        guard let pixelValues,
              let frames = combinedFrames(imageFrames: imageFrames, videoFrames: videoFrames)
                .nilIfEmpty else {
            throw DFlashError.unsupportedGeneration("多模態 DFlash prefill 缺少視覺輸入。")
        }
        ...
        return languageModel.dflashForward(
            inputIds: inputIds,

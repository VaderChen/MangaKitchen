...
public final class VLMModelFactory: GenericModelFactory {
    ...
    public func _load(
        ...
        async let processorConfigTask = loadProcessorConfig(
            ...
            modelConfigurationData: configData
        )
...
private func loadProcessorConfig(
    from modelDirectory: URL,
    modelType: String,
    modelConfigurationData: Data
) async throws -> (Data, BaseProcessorConfiguration) {
    ...
    for url in [preprocessorConfigURL, processorConfigURL]
    ...
    if let fallback = qwen35ProcessorConfigurationData(
        modelType: modelType,
...
private func qwen35ProcessorConfigurationData(
    modelType: String,
    modelConfigurationData: Data
) -> Data? {
    guard modelType == "qwen3_5" || modelType == "qwen3_5_moe",
          let root = try? JSONSerialization.jsonObject(with: modelConfigurationData)
            as? [String: Any],
          let visionConfiguration = root["vision_config"] as? [String: Any]
    else { return nil }

    let imageMean = jsonFloatArray(
        root["image_mean"]
    ) ?? [0.5, 0.5, 0.5]
    let imageStd = jsonFloatArray(
        root["image_std"]
    ) ?? [0.5, 0.5, 0.5]
    let patchSize = jsonInteger(root["patch_size"])
        ?? jsonInteger(visionConfiguration["patch_size"])
        ?? 16
    let temporalPatchSize = jsonInteger(root["temporal_patch_size"])
        ?? jsonInteger(visionConfiguration["temporal_patch_size"])
        ?? 2
    let mergeSize = jsonInteger(root["merge_size"])
        ?? jsonInteger(root["spatial_merge_size"])
        ?? jsonInteger(visionConfiguration["spatial_merge_size"])
        ?? 2
    let size = root["size"] as? [String: Any]
    let minPixels = jsonInteger(root["min_pixels"])
        ?? jsonInteger(size?["shortest_edge"])
        ?? 65_536
    let maxPixels = jsonInteger(root["max_pixels"])
        ?? jsonInteger(size?["longest_edge"])
        ?? 16_777_216

    let processor: [String: Any] = [
        "processor_class": root["processor_class"] as? String ?? "Qwen3VLProcessor",
        "image_processor_type": root["image_processor_type"] as? String
            ?? "Qwen3VLImageProcessor",
        "image_mean": imageMean,
        "image_std": imageStd,
...
private func jsonFloatArray(_ value: Any?) -> [Float]? {
    guard let values = value as? [Any] else { return nil }
    let result = values.compactMap { ($0 as? NSNumber).map { $0.floatValue } }

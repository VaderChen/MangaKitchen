...
public protocol DFlashTargetModel: LanguageModel {
    ...
    /// Variant used by targets that need model-specific state after a
    /// multimodal prefill, such as Qwen's M-RoPE position state.
    func dflashForward(
        _ inputs: MLXArray,
        cache: [KVCache]?,
        captureLayerIDs: [Int],
...
/// Target contract for a VLM's first prefill. The implementation must merge
/// visual features into the text embeddings, calculate model-specific visual
/// positions, and return the same intermediate hidden states used by DFlash
/// verification.
public protocol DFlashMultimodalTargetModel: DFlashTargetModel {
    func dflashMultimodalPrefill(
        input: LMInput,
        cache: [KVCache]?,
        captureLayerIDs: [Int],
        captureRollback: Bool

...
public struct DFlashSpeculativeTokenIterator: TokenIteratorProtocol {
    ...
    public init(
        ...
        guard input.audio == nil else {
            throw DFlashError.unsupportedGeneration("DFlash 目前不支援音訊輸入。")
        }
        guard input.text.tokens.ndim == 1
                || (input.text.tokens.ndim == 2 && input.text.tokens.dim(0) == 1) else {
        ...
        anchor = input.text.tokens.ndim == 1
            ? input.text.tokens
            : input.text.tokens[0, 0...]
    ...
    mutating func prepare(input: LMInput, prefillStepSize: Int) throws {
        let prompt = input.text.tokens.ndim == 1
            ? input.text.tokens
            : input.text.tokens[0, 0...]
        ...
        try withPreparedCache(targetCache, lengths: input.text.sequenceLengths) {
            if input.image != nil || input.video != nil {
                guard let multimodalTarget = target as? any DFlashMultimodalTargetModel else {
                    throw DFlashError.unsupportedTarget(
                        "(type(of: target)) 未提供多模態 DFlash prefill 契約。")
                }
                let output = try multimodalTarget.dflashMultimodalPrefill(
                    input: input,

...
- 全域設定 DLG 分為一般、進階、模型、MCP、關於；可設定介面語言、UI 色系、CPU／GPU 圖像合成、資料位置、多模態／文字定位／OCR／文生文／超高解析度模型、MCP 連接埠與 IP／CIDR 白名單。
...
- 翻譯方式可選已下載的文生文或多模態模型。文生文為預設，只接收 OCR／VLM 已抽出的原文，不讀取頁面圖片；多模態則可同時利用畫面語境，也能選作文字定位與原文抽取。因此多模態 VLM 已不是整個工作流的必要條件。
...
- 文生文／多模態模型的翻譯 Prompt 與嚴格 JSON 回傳解析。
- 可直接載入本機 Hugging Face MLX 文生文與多模態模型目錄，由 `mlx-swift-lm` 在 Apple Silicon／Metal 執行。
...
3. **翻譯與排版**：GUI 以內建 OCR 或所選 VLM 路徑抽取原文，再由所選文生文或多模態模型執行翻譯；可選二次校稿與語意 QA 沿用同一翻譯路徑。MCP 則由 Agent 依 App 提供的工作包完成原文、翻譯與排版，結果由 App 回寫專案狀態。
...
在「設定 → 模型」下載文生文或多模態翻譯模型。區域辨識、原文抽取、翻譯、背景修補與合成都在 Mac 本機執行；模型完成下載後，工作流不需要把漫畫內容送到外部 AI 服務。

- 文生文翻譯只使用 OCR／VLM 已抽出的原文，不讀取圖片。`imageToText` VLM 只在專案明確選擇 VLM 整區原文抽取或多模態翻譯時才是必要條件；步驟二遮罩、PP-OCR「重新抽字」、文生文翻譯、重新排版與輸出都不需要 VLM。App 不使用 Apple Vision OCR，擬聲字仍不進入目前的翻譯主流程。
...
文生文與多模態分別由 `MLXTextRuntime`、`MLXVLMRuntime` 載入 `model_type` 受 `mlx-swift-lm` 支援的本機模型。完整的 Hugging Face MLX 目錄若包含 `config.json`、Tokenizer 與 Safetensors 權重，App 會依是否具有影像／視覺欄位自動判定能力，不強制要求 `mangakitchen-model.json`。若需要自訂顯示名稱或生成參數，仍可放置 manifest 覆寫自動設定。

文生文建議 `mlx-community/Qwen3-4B-4bit`；多模態建議先使用約 3GB 的 `lmstudio-community/Qwen3.5-4B-MLX-4bit`：
...
已封裝版本的英文更新紀錄請見 [MangaKitchen 1.26.0823 build 0323 Release Notes](Documentation/RELEASE_NOTES_1.26.0823-build-0323.md)；封裝後的新變更請見 [Development Release Notes](Documentation/RELEASE_NOTES_UNRELEASED.md)。

## 已知邊界

- 內建氣泡 BBOX、PP-OCRv6 Medium 預設 OCR 與 Small fallback OCR Core ML 模型，均由 Apache-2.0 上游模型轉換而來；原始圖生文權重、超解析與圖生圖模型仍需由使用者另行下載與設定。OCR `.mlpackage`、字元表與相鄰的 Apache-2.0 授權聲明已納入 repository。

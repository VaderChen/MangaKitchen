...
- 翻譯步驟可選擇加入原生 Swift／Core ML OCR。VLM 仍負責文字定位與區域分類；OCR 只在逐區或整頁翻譯前產生候選原文，並以模型 ID 分開保存，不覆寫 VLM 原文、文字座標或遮罩。複合 OCR 與二次校稿融合保留給後續流程，低信心結果不會自動取代目前原文。
...
- 內建氣泡 BBOX Core ML 模型；圖生文、OCR、超解析與圖生圖模型權重仍需由使用者另行下載與設定。OCR 字元表會隨原始碼保存，編譯後的 `.mlpackage` 不納入 repository。

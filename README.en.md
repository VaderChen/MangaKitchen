...
- Global Settings dialog with General, Advanced, Models, MCP, and About tabs for language, color scheme, data location, model paths, MCP port, and IP/CIDR allowlist.
...
- Normalized vector brush strokes for adding, erasing, and undoing masks before generating a binary PNG.
- Vision OCR, horizontal/vertical line grouping, and comic reading-order resolution.
...
- Dialogue masks and image-to-image restoration, with a Metal Compute neighborhood fallback when no image model is installed.
- Core Text automatic shrinking, horizontal/vertical typesetting, and relayout after manual edits.
...
2. **Text and masks**: detect dialogue regions and source text, generate masks, then let a user or Agent add, erase, or correct them.
...
- Before step 3, validate the source page, text regions, OCR source text, and mask. Incomplete data falls back to step 2.
...
- The `imageToText` model performs page-context translation in step 3 and is required for automatic full-page processing.
- The `imageToImage` model performs background restoration in step 4 and is optional; Metal Compute restoration is used when it is not configured.
...
3. Read the page, source image, or `.str` resource, translate with the Agent, and write every region's translation and typesetting settings through `mangakitchen.region.update`.
4. Call `mangakitchen.page.compose` to restore the background and generate output.

If a local `imageToText` model is also loaded, the Agent may call `mangakitchen.page.translate`, or orchestrate steps 2–4 with `mangakitchen.page.run_full`. `page.run_full` requires a local image-to-text model; pure Agent translation should use `detect_masks → region.update → compose`.
...
- OCR geometry currently uses general-purpose grouping. Precise balloon contours, narration boxes, and sound-effect classification can later use segmentation while preserving `DialogueRegion` output.
